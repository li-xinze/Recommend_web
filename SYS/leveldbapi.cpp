...
void LeveldbAPI::readMessage()
{
    ...
    string string1 = strlist[0].toStdString();
    string string2 = strlist[1].toStdString();
    string string3 = strlist[2].toStdString();
    cout<<string1<<endl;
    cout<<string2<<endl;
    cout<<string3<<endl;
    sendMessage();
}
void LeveldbAPI::sendMessage(){
    QString ok = "ok";
    QByteArray datagram = ok.toUtf8();
...
string LeveldbAPI::SwitchTmdb_Movie(string a){
    leveldb::DB *db = nullptr;
    leveldb::Options options;
    // 如果数据库不存在也不创建
    options.create_if_missing = false;
    // 创建的数据库在 //Tmdb_Title
    leveldb::Status status = leveldb::DB::Open(options, "/SmallTmdb_Movie", &db);
    assert(status.ok());
    string TmdbID = a;
    string key1 = TmdbID;
    string get_value1;
...
void LeveldbAPI::StoreNewRating(string& a,string& b,string& c){
    leveldb::DB *db = nullptr;
    leveldb::Options options;
    options.create_if_missing = false;
    // 创建的数据库在 //Tmdb_Title
    leveldb::Status status = leveldb::DB::Open(options, "/User", &db);
    assert(status.ok());
    string UserID = a;
    string MovieID = SwitchTmdb_Movie(b);
    string Rating = c;
    /*stringstream ss1;
    ss1 << "User" << UserID << ":" << "Movie" << MovieID << ":" << "Rating";
    string key1 = ss1.str();
    string value1 = Rating;
    string get_value1;
    leveldb::Status s1 = db->Put(leveldb::WriteOptions(), key1, value1);
    // 写入成功，就读取 key:people 对应的 value
    if  ( s1.ok() )
